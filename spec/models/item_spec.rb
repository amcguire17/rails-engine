...
describe Item do
  ...
  describe 'class methods' do
    before :each do
      @item1 = create(:item, name: 'Hand Sanitizer', unit_price: 25.00)
      @item2 = create(:item, name: 'Hand Lotion', unit_price: 10.00)
      @item3 = create(:item, name: 'Soap', unit_price: 17.50)
    end
    ...
    describe '.quantity_by_revenue' do
      it 'can return specific amount of items by revenue' do
        ...
        create(:invoice_item, item: items.first, invoice: Invoice.first, quantity: 2, unit_price: 10.00)
        create(:invoice_item, item: items.second, invoice: Invoice.second, quantity: 1, unit_price: 15.00)
        create(:invoice_item, item: items.third, invoice: Invoice.third, quantity: 3, unit_price: 20.00)
        create(:invoice_item, item: items.fourth, invoice: Invoice.fourth, quantity: 1, unit_price: 13.00)
        create(:invoice_item, item: items.fifth, invoice: Invoice.fifth, quantity: 3, unit_price: 7.00)
